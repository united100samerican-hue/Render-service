...
class AudioService:
    ...
    async def _connect(self)->None:
        ...
        if PyTgCalls is None:raise RuntimeError('pytgcalls_import_failed')
        ...
        if self.calls is None:self.calls=PyTgCalls(self._client)
    ...
    async def _play(self,chat_id:int,source:str)->None:
        ...
        played=False
        for kwargs in ({},{'file':source}):
            try:
                if kwargs:res=self.calls.play(chat_id,**kwargs)
                else:res=self.calls.play(chat_id,source)
                await self._maybe_await(res);played=True;break
            except TypeError:
                continue
            except Exception as exc:
                name=type(exc).__name__
                msg=str(exc)
                if 'NoActiveGroupCall' in name or 'NoActiveGroupCall' in msg:
                    raise RuntimeError('voice_chat_not_found') from exc
                raise
        if not played:raise RuntimeError('pytgcalls_play_method_unavailable')
    ...
    async def start(self,chat_id:int,source_type:str,source_id:str,title:str='',duration:int=0,offset:int=0,source_chat_id:int=0,source_message_id:int=0)->dict[str,Any]:
        ...
        async with self._lock_for(chat_id):
            ...
            try:
                path,video=await (self._download_telegram_message(source_chat_id,source_message_id,st,title) if st=='telegram_message' else self._download_telegram_file(source_id,st,title));new_path=str(path)
                probed=await self._probe_duration(path)
    ...
    async def pause(self,chat_id:int)->dict[str,Any]:
        async with self._lock_for(chat_id):
            ...
            try:
                ...
                if self.calls is None:raise RuntimeError('pytgcalls_not_ready')
                await self._maybe_await(self.calls.pause(chat_id));s.status='paused';s.paused=True;self._sessions[chat_id]=self._touch(s);return{'ok':True,'action':'pause','state':self.state(chat_id)}
    ...
    async def resume(self,chat_id:int)->dict[str,Any]:
        async with self._lock_for(chat_id):
            ...
            try:
                await self._ensure_owner()
                if self.calls is None:raise RuntimeError('pytgcalls_not_ready')
                await self._maybe_await(self.calls.resume(chat_id));s.status='playing';s.paused=False;self._sessions[chat_id]=self._touch(s);return{'ok':True,'action':'resume','state':self.state(chat_id)}
    ...
    async def seek(self,chat_id:int,delta:int=0,position:int|None=None)->dict[str,Any]:
        async with self._lock_for(chat_id):
            ...
            try:
                ...
                if self.calls is None:raise RuntimeError('pytgcalls_not_ready')
                await self._maybe_await(self.calls.play(chat_id,new_path))
        ...
        async with self._lock_for(chat_id):
            ...
            try:
                ...
                if self.calls is not None:
                    try:
                        leave=getattr(self.calls,'leave_call',None) or getattr(self.calls,'leave_group_call',None)
                        if callable(leave):
                            await self._maybe_await(leave(chat_id));left=True
                    except Exception as exc:logger.debug('pytgcalls leave failed: %s',exc)
                ...
                self._sessions.pop(chat_id,None);return{'ok':left,'action':'stop','stopped':left,'left_call':left,'state':self.state(chat_id)}
