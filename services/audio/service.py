...
class AudioService:
    ...
    async def start(self,chat_id:int,source_type:str,source_id:str,title:str='',duration:int=0,offset:int=0,source_chat_id:int=0,source_message_id:int=0)->dict[str,Any]:
        ...
        async with self._lock_for(chat_id):
            ...
            try:
                path,video,probed=await (self._download_telegram_message(source_chat_id,source_message_id,st,title) if st=='telegram_message' else self._download_telegram_file(source_id,st,title));new_path=str(path)
