...
class AudioService:
    ...
    async def ensure_ready(self):
        ...
        if not API_ID or not API_HASH or not SESSION_STRING:
            self.backend_error='missing_env: API_ID/API_HASH/SESSION_STRING'; raise RuntimeError(self.backend_error)
        self.client=TelegramClient(StringSession(SESSION_STRING), API_ID, API_HASH)
        await self.client.connect()
        if not await self.client.is_user_authorized():
            self.backend_error='session_not_authorized'; raise RuntimeError(self.backend_error)
        self.calls=CallManager(self.client)
        await self.calls.start()
        self.telegram_media=TelegramMedia(BOT_TOKEN, self.client, self.root)
        self.ready=True
        self.backend_error=''
        log.info('ready')
    ...
    async def close(self):
        for chat_id in list(self.sessions):
            try: await self.calls.stop(chat_id) if self.calls else None
    ...
    async def call_state(self, chat_id: int) -> dict[str,Any]:
        ...
        try: active=await self.calls.active(int(chat_id))
            ...
            msg=str(e)
            if 'GROUPCALL_INVALID' in msg or 'NoActiveGroupCall' in msg: active=False
            else: raise
    ...
    async def _resolve(self, chat_id:int, source_type:str, source_id:str, title:str='', source_chat_id:int=0, source_message_id:int=0) -> dict[str,Any]:
        ...
        if st in {'url','link','youtube','yt'}:
            r=await self.urls.resolve(source_id)
            r.update(source_type='url',source_id=source_id)
            return r
    ...
    async def meta(self, chat_id:int, source_type:str, source_id:str, **kw) -> dict[str,Any]:
        ...
        async with self.lock(chat_id):
            ...
            return {'ok':True,'action':'meta','state':{
                'chat_id':chat_id,'source_type':str(r.get('source_type') or source_type),
                'source_id':str(r.get('source_id') or source_id),'title':str(r.get('title') or kw.get('title') or 'غير معروف'),
                'duration':int(r.get('duration') or kw.get('duration') or 0),'video':bool(r.get('video')),
                'media_kind':str(r.get('media_kind') or ('video' if r.get('video') else 'audio')),
                'webpage_url':str(r.get('webpage_url') or r.get('source_url') or source_id),
                'source_url':str(r.get('source_url') or source_id),'thumbnail':str(r.get('thumbnail') or ''),
    ...
    async def _start_locked(self, chat_id:int, source_type:str, source_id:str, title:str='', duration:int=0, offset:int=0, source_chat_id:int=0, source_message_id:int=0) -> dict[str,Any]:
        ...
        if old and old.local_path and old.local_path != str(r.get('local_path') or ''): self._clean_file(old.local_path)
        stream=str(r.get('stream_url') or '')
        if not stream: raise RuntimeError('stream_url_missing')
        try:
            await self.calls.play(chat_id,stream)
        except Exception as e:
            msg=str(e)
            if 'NoActiveGroupCall' in msg or 'GROUPCALL_INVALID' in msg: raise AudioServiceError('no_active_call','no_active_call')
            raise
        now=self._now()
    ...
    async def stop(self, chat_id:int):
        ...
        async with self.lock(chat_id):
            ...
            try: await self.calls.stop(chat_id)
            finally:
                self._clean_file(s.local_path if s else '')
    ...
    async def resume(self,chat_id:int):
        ...
        async with self.lock(chat_id):
            ...
            await self.calls.resume(chat_id); now=self._now(); s.started_at=now-s.position; s.status='playing'; s.updated_at=now
    ...
    async def enqueue(self,chat_id,source_type,source_id,**kw):
        # Worker owns the persistent playlist. Render remains a single-track transport.
