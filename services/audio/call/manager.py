...
class CallManager:
 def __init__(self,client:Any):self.client=client;self.calls=PyTgCalls(client)
 ...
 async def play(self,chat_id:int,source:str,video:bool=False,offset:int=0)->None:
  ...
  ffmpeg=f"-ss {max(0,int(offset))}" if int(offset)>0 else ""
  if video:
   stream=MediaStream(str(source),AudioQuality.HIGH,VideoQuality.HD_720p,ffmpeg_parameters=ffmpeg)
  else:
   stream=MediaStream(str(source),AudioQuality.HIGH,video_flags=MediaStream.Flags.IGNORE,ffmpeg_parameters=ffmpeg)
  await self.calls.play(int(chat_id),stream)
 async def pause(self,chat_id:int)->None:await self.calls.pause(int(chat_id))
 async def resume(self,chat_id:int)->None:await self.calls.resume(int(chat_id))
