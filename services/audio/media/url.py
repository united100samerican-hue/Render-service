...
class UrlResolver:
 def __init__(self):
  ...
  self._cache={}
  self._cookie_file=""
  self._cookie_status={"configured":False,"loaded":False,"valid_format":False,"source":"none","bytes":0,"youtube_domains":0,"cookie_rows":0,"auth_cookies":0,"expired_auth_cookies":0}
  self._pot_available=None
  self._pot_version=""
  self._pot_checked_at=0.0
  self._yt_dlp_auth_checked_at=0.0
  self._yt_dlp_auth_status={"checked":False,"detected":False,"login_info":False,"sapisid":False,"cookie_count":0,"error":""}
  self._prepare_cookies()

 def _cache_get(self,source):
  item=self._cache.get(source)
  if not item:return None
  if time.time()-item[0]>self._cache_ttl:
   self._cache.pop(source,None)
   return None
  return dict(item[1])

 def invalidate(self,source):
  self._cache.pop(str(source or "").strip(),None)

 def _cache_set(self,source,value):
  self._cache[source]=(time.time(),dict(value))
  if len(self._cache)>128:
   old=min(self._cache.items(),key=lambda x:x[1][0])[0]
 ...
 def _decode_cookie_text(raw):
  ...
  if value.startswith("base64:"):
   try:
    return base64.b64decode(re.sub(r"\s+","",value[7:]),validate=False).decode("utf-8-sig"),"base64"
   except Exception:
    return "","base64_invalid"
  if value.startswith("data:text/plain;base64,"):
   try:
    return base64.b64decode(re.sub(r"\s+","",value.split(",",1)[1]),validate=False).decode("utf-8-sig"),"base64"
   except Exception:
    return "","base64_invalid"
 ...
 def _cookie_stats(text):
  ...
  first=next((x.lstrip("\ufeff").strip() for x in lines if x.strip()),"")
  domains=set()
  rows=auth=expired=login=sapi=0
  ...
  return {
   "valid_format":first in {"# Netscape HTTP Cookie File","# HTTP Cookie File"},
   "cookie_rows":rows,
   "youtube_domains":len(domains),
   "auth_cookies":auth,
   "expired_auth_cookies":expired,
   "login_info":login>0,
   "sapisid_cookies":sapi
  }
 ...
 def _prepare_cookies(self):
  ...
  try:
   ...
   os.chmod(path,0o600)
   MozillaCookieJar(path).load(ignore_discard=True,ignore_expires=True)
   ...
   try:
    ...
    pass
   self._cookie_status.update(valid_format=False,load_error=type(exc).__name__)
   return
  ...
  log.info(
   "youtube cookies configured source=%s valid=%s bytes=%d youtube_domains=%d auth=%d expired_auth=%d login_info=%s sapisid=%d yt_dlp_auth=%s yt_dlp_login=%s yt_dlp_sapisid=%s",
   source,
   bool(self._cookie_file),
   self._cookie_status["bytes"],
   self._cookie_status["youtube_domains"],
   self._cookie_status["auth_cookies"],
   self._cookie_status["expired_auth_cookies"],
   self._cookie_status.get("login_info"),
   self._cookie_status.get("sapisid_cookies",0),
   self._yt_dlp_auth_status.get("detected"),
   self._yt_dlp_auth_status.get("login_info"),
   self._yt_dlp_auth_status.get("sapisid")
  )

 def _check_yt_dlp_cookie_auth(self,force=False):
  now=time.time()

  if not force and self._yt_dlp_auth_status.get("checked") and now-self._yt_dlp_auth_checked_at<60:
   return dict(self._yt_dlp_auth_status)
  ...
  if not self._cookie_file:
   self._yt_dlp_auth_status=result
   self._yt_dlp_auth_checked_at=now
   return result
  ...
  try:
   with yt_dlp.YoutubeDL({
    ...
    yt_cookies=[
     c for c in cookies
     if str(getattr(c,"domain","")).lower().lstrip(".")=="youtube.com"
     or str(getattr(c,"domain","")).lower().endswith(".youtube.com")
    ]
    ...
    result["sapisid"]=any(
     bool(names.get(n))
     for n in ("SAPISID","__Secure-1PAPISID","__Secure-3PAPISID")
    )
 ...
 def cookie_status(self):
  ...
  status["yt_dlp"]=getattr(yt_dlp.version,"__version__","unknown")
  return status
 ...
 def _is_youtube_url(value):
  ...
  if not raw.lower().startswith(("http://","https://")):
   return False
  try:
   return (urlsplit(raw).hostname or "").lower().rstrip(".") in YOUTUBE_HOSTS
  except Exception:
   return False

 @staticmethod
 def _is_youtube_search(value):
 ...
 def youtube_video_id(value):
  ...
  if not raw:
   return ""
  if raw.lower().startswith(SEARCH_PREFIXES):
   return ""
 ...
 def _is_direct(url,content_type=""):
  ...
  content=content_type.lower().split(";",1)[0].strip()
 ...
 def _provider_available(self):
  ...
  if self._pot_available is not None and now-self._pot_checked_at<10:
   return bool(self._pot_available)

  self._pot_version=""

  try:
   r=httpx.get(f"{POT_PROVIDER_URL.rstrip('/')}/ping",timeout=.8)
   payload=r.json() if r.content else {}
   self._pot_version=str(payload.get("version") or "").strip()
   self._pot_available=r.status_code==200 and bool(self._pot_version)
  except Exception as exc:
   self._pot_available=False
   log.warning("youtube pot provider unavailable error=%s",type(exc).__name__)

  self._pot_checked_at=now
  return bool(self._pot_available)

 def _youtube_options(self,player_clients,use_provider=False,use_cookies=True,download=False,outdir="",player_skip=None):
  ...
  opts={
   ...
   "format":"best[height<=720][vcodec!=none][acodec!=none]/best[vcodec!=none][acodec!=none]/18/best[acodec!=none]/best"
  }

  yargs={
   "player_client":[str(x).strip() for x in player_clients if str(x).strip()]
  }

  if use_provider:
   yargs["fetch_pot"]=["always"]

  if trace and use_provider:
   yargs["pot_trace"]=["true"]

  if player_skip:
   yargs["player_skip"]=[
    str(x).strip()
    for x in player_skip
    if str(x).strip()
   ]
  ...
  deno=shutil.which("deno") or (
   "/usr/local/bin/deno"
   if Path("/usr/local/bin/deno").is_file()
   else ""
  )
 ...
 def _attempts(self,source):
  provider=self._provider_available()
  a=[]
  search=self._is_youtube_search(source)

  if self._cookie_file:
   if provider and not search:
    a.append((
     "mweb-cookies+pot-skip-webpage",
    ...
    a.append((
     "web_safari-cookies+pot",
     self._youtube_options(["web_safari"],True,True)
    ))

    a.append((
     "web-cookies+pot",
     self._youtube_options(["web"],True,True)
    ))

    a.append((
     "web_creator-cookies+pot",
     self._youtube_options(["web_creator"],True,True)
    ))

    a.append((
     "default-cookies+pot",
     self._youtube_options(["default"],True,True)
    ))
   else:
    a.append((
     "default-cookies"+("+pot" if provider else ""),
     self._youtube_options(["default"],provider,True)
    ))

    if not search:
     a.append((
      "web_safari-cookies",
      self._youtube_options(["web_safari"],False,True)
     ))

   a.append((
    "web_embedded-cookies",
    self._youtube_options(["web_embedded"],False,True)
   ))

  if search:
   a.append((
    "default-guest",
    self._youtube_options(["default"],False,False)
   ))
  else:
   a.append((
    "android_vr-guest",
    self._youtube_options(["android_vr"],False,False)
   ))

   a.append((
    "tv-guest",
    self._youtube_options(["tv"],False,False)
   ))
 ...
 def _pick_entry(info):
  if info and info.get("entries"):
   return next((e for e in info["entries"] if e),None)
  return info
 ...
 def _video_flag(info):
  ...
  if rf:
   vc=any(
    str(x.get("vcodec") or "") not in {"","none"}
    for x in rf
   )
   ac=any(
    str(x.get("acodec") or "") not in {"","none"}
    for x in rf
   )
   return vc and ac

  return (
   str(info.get("vcodec") or "") not in {"","none"}
   and str(info.get("acodec") or "") not in {"","none"}
  )

 @staticmethod
 def _find_downloaded(outdir,video_id):
  d=Path(outdir)
  bad=(".part",".ytdl",".temp",".info.json",".jpg",".webp",".png")

  files=[
   p for p in d.glob(f"{video_id}.*")
   if p.is_file()
   and not p.name.endswith(bad)
  ...
  files.sort(
   key=lambda p:(pref.get(p.suffix.lower(),50),-p.stat().st_mtime)
  )
 ...
 def _extract(self,source,download=False,outdir=""):
  ...
  attempts=(
   ...
   if is_yt
   else [("generic",self._generic_options())]
  )
  ...
  if is_yt:
   ...
   log.info(
    "youtube extraction source=%s attempts=%d yt_dlp=%s provider=%s provider_version=%s cookies=%s yt_dlp_auth=%s yt_dlp_login=%s yt_dlp_sapisid=%s download=%s",
    source[:120],
    len(attempts),
  ...
  for i,(label,base_opts) in enumerate(attempts):
   ...
   if download and is_yt:
    yargs=base_opts.get("extractor_args",{}).get("youtube",{})

    opts=self._youtube_options(
     yargs.get("player_client") or ["default"],
     "youtubepot-bgutilhttp" in base_opts.get("extractor_args",{}),
     "cookiefile" in base_opts,
     outdir=outdir,
     download=True,
     player_skip=yargs.get("player_skip")
    )

   try:
    with yt_dlp.YoutubeDL(opts) as ydl:
     info=self._pick_entry(
      ydl.extract_info(source,download=download)
     )
     ...
     if download and is_yt:
      ...
      path=self._find_downloaded(outdir,vid)

      if not path:
       raise RuntimeError("youtube_download_file_missing")

    if is_yt:
     log.info(
      "youtube extraction succeeded attempt=%d/%d client=%s yt_dlp=%s provider=%s cookies=%s download=%s",
      i+1,
      len(attempts),
      label,
      getattr(yt_dlp.version,"__version__","unknown"),
    ...
    if is_yt:
     ...
     log.warning(
      "youtube extraction failed attempt=%d/%d client=%s retryable=%s yt_dlp_auth=%s error=%s",
      i+1,
      len(attempts),
      label,
      retryable,
    ...
    if not is_yt or i>=len(attempts)-1 or not retryable:
     raise
  ...
  if not stream:
   formats=[
    x for x in (info.get("formats") or [])
    if x.get("url") and x.get("protocol")!="mhtml"
   ]
  ...
  result={
   ...
   "video_id":vid,
   "http_headers":allowed,
   "remote_stream":False
  }

  if download and is_yt:
   result["local_path"]=path
   result["stream_url"]=path
   result["remote_stream"]=False
 ...
 async def resolve(self,url,download=False,output_dir=""):
  ...
  if cached and (
   ...
   or (
    ...
    and Path(str(cached["local_path"])).is_file()
   )
  ):
   return cached
  ...
  if is_yt or not self._is_direct(source)[0]:
   ...
   if not direct:
    ctype,final=await self._head(source)
    direct,kind,live=self._is_direct(final,ctype)
