...
class TikTokReceiver:
    def __init__(self) -> None:
        ...
        self.ydl = YoutubeDL({
            ...
            "extract_flat": False,
            "http_headers": {
                "User-Agent": getattr(cfg, "user_agent", "Mozilla/5.0") or "Mozilla/5.0",
            },
        })
    ...
    async def _extract(self, chat_id: int, url: str) -> dict[str, Any]:
        for attempt in range(1, 6):
            try:
                info = await asyncio.to_thread(self.ydl.extract_info, url, False)
                if not info:
                ...
                for fmt in formats:
                    ...
                    if vcodec != "none":
                        if best_video is None or int(fmt.get("height") or 0) > int(best_video.get("height") or 0):
                            best_video = fmt

                    if acodec != "none":
                        if best_audio is None or int(fmt.get("abr") or 0) > int(best_audio.get("abr") or 0):
                            best_audio = fmt

                if best_video:
                    video_url = best_video.get("url") or best_video.get("manifest_url") or ""
                if best_audio:
                    audio_url = best_audio.get("url") or best_audio.get("manifest_url") or ""
                ...
                log.info("Resolved TikTok stream chat=%s title=%s", chat_id, title)
                ...
                log.warning("yt-dlp attempt %s failed: %s", attempt, e)
                if attempt == 5:
    ...
    async def refresh(self, *, chat_id: int, url: str) -> dict[str, Any]:
        self.cache.pop(url, None)
        return await self.resolve(chat_id=chat_id, url=url, force=True)
